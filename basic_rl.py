...
# Adapted from https://gym.openai.com/evaluations/eval_xSOlwrBsQDqUW7y6lJOevQ/
...
parser.add_argument(
    "-al", "--alpha", default="0.5", type=float, help="Learning rate. (Default: 0.1)"
)
parser.add_argument(
    "-be",
    "--beta",
...
parser.add_argument(
    "-ga", "--gamma", default="0.99", type=float, help="Discount rate. (Default: 0.99)"
)
parser.add_argument(
    "-ep",
    "--epsilon",
...
def main():
    ...
    result_dir = "results-{0}-{1}-{2}-ga-{3}-ep-{4}-ed-{5}-al-{6}".format(
        env_type, algorithm_type, policy_type, gamma, epsilon, epsilon_decay, alpha
    )

    # Start monitoring the simulation for OpenAI Gym
    MyWrapper.Monitor(env, result_dir, force=True)
    ...
    for i_episode in range(n_episode):
        ...
        for i_step in range(max_step):
            ...
            if done and (r == 0.0):
                # Punishment for falling into a hole
                r = 0.0
            elif not done:
                # Cost per step
                r = -0.01
            ...
            if done:
                if policy_type == "softmax":
                    print(
                        "Episode: {0}\t Steps: {1:>4}\tCumuR: {2:>5.2f}\tTermR: {3}\tBeta: {4:.3f}".format(
                            i_episode, i_step, cumu_r, r, beta
                        )
                    )
                    history.append([i_episode, i_step, cumu_r, r, beta])
                elif policy_type == "epsilon_greedy":
                    print(
                        "Episode: {0}\t Steps: {1:>4}\tCumuR: {2:>5.2f}\tTermR: {3}\tEpsilon: {4:.3f}".format(
                            i_episode, i_step, cumu_r, r, epsilon
                        )
                    )
    ...
    ax[0, 0].plot(
        history[window_size - 1 :, 0], running_average(history[:, 1], window_size)
    )
    # Cumulative reward
    ...
    ax[0, 1].plot(
        history[window_size - 1 :, 0], running_average(history[:, 2], window_size)
    )
    # Terminal reward
    ...
    ax[1, 0].plot(
        history[window_size - 1 :, 0], running_average(history[:, 3], window_size)
    )
    # Epsilon/Beta
